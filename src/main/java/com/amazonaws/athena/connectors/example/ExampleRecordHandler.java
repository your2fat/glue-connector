...
//        dataBucket = "aws-glue-assets-481926336056-eu-central-1";
//        dataBucket = System.getenv("data_bucket");

        logger.info("JOB_NAME: {}", System.getenv("JOB_NAME"));

        GetJobResult currentJob = AWSGlueClientBuilder.defaultClient().getJob(new GetJobRequest().withJobName(System.getenv("JOB_NAME")));

        logger.info("currentJob: {}", currentJob);

        String dataBucket = currentJob.getJob().getDefaultArguments().get("--data_bucket");

        logger.info("data_bucket: {}", dataBucket);
